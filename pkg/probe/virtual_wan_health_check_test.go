...
func TestVirtualWANHealthCheckSelect(t *testing.T) {
	c := newFakeClient()
	c.prepare("api/v2/monitor/virtual-wan/health-check/select", "testdata/virtual_wan_health_check.jsonnet")
	r := prometheus.NewPedanticRegistry()
	meta := &TargetMetadata{
		VersionMajor: 7,
		VersionMinor: 6,
	}
	if !testProbeWithMetadata(probeVirtualWANHealthCheck, c, meta, r) {
		t.Errorf("probeVirtualWANHealthCheck() returned non-success")
	}
